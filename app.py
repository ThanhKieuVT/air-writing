...
def main():
    ...
    # Model load #############################################################
    #sử dụng mô hình MediaPipe Hands từ thư viện mediapipe để phát hiện bàn tay trong khung hình video
#     hands = mp_hands.Hands(...): Dòng lệnh này khởi tạo đối tượng Hands từ mô hình MediaPipe Hands. Các tham số được truyền vào có ý nghĩa:
#     static_image_mode (gán giá trị từ use_static_image_mode): Xác định chế độ hoạt động của mô hình. True là chế độ hình ảnh tĩnh, False là chế độ video (mặc định).
# max_num_hands (gán giá trị 1): Số bàn tay tối đa được phát hiện trong mỗi khung hình.
# min_detection_confidence (gán giá trị từ min_detection_confidence): Độ tin cậy tối thiểu để phát hiện bàn tay (giá trị từ 0.0 đến 1.0).
# min_tracking_confidence (gán giá trị từ min_tracking_confidence): Độ tin cậy tối thiểu để theo dõi bàn tay giữa các khung hình (giá trị từ 0.0 đến 1.0).
    ...
    with open('model/handwritten_classifier/handwriting_vi_chharacter.csv',
            encoding='utf-8-sig') as f:
    ...
    while True:
        ...
        if key == 115:
            cv.imwrite(os.path.join(custom_data_path, 'i%s.jpg' % next_index_img), img2predict)
            next_index_img += 1
            ...
            for hand_landmarks, handedness in zip(results.multi_hand_landmarks,
                ...
                if hand_sign_id == 4:  # Point gesture
                    ...
                    if hand_sign_id == 0:
                        ...
                        predicted_character = handwritten_classifier_labels[handwritten_classifier(img2predict)]
                        label = str(handwritten_classifier(img2predict))
                        predicted_character_utf8 = predicted_character.encode('utf-8')
                        print("Predicted character : %s" % predicted_character_utf8.decode('utf-8'))
        ...
        frame = draw_predicted_character(frame, label)
...
def draw_predicted_character(image, predicted_character):
    cv.putText(image, "Predicted label: " + predicted_character, (10, 30),
